...
#            lvl2EnemyRect = pygame.Rect(lvl2ExPos, lvl2EyPos, lvl2EW, lvl2EH) #initalises the enemies
#            lvl3EnemyRect = pygame.Rect(lvl3ExPos, lvl3EyPos, lvl3EW, lvl3EH) #initalises the enemies
    ...
    lvl1radius = random.randint(1,5)
    lvl1ExPos = random.randint(player.getCoords()[0],player.getCoords()[0] + lvl1radius)
    lvl1EyPos = random.randint(player.getCoords()[1],player.getCoords()[1] + lvl1radius)
    distXlvl1 = player.xPos - lvl1ExPos
    distYlvl1 = player.yPos - lvl1EyPos
    totaldistlvl1 = math.hypot(distXlvl1, distYlvl1)
    

    
 
    lvl2radius = random.randint(1,5)
    lvl2ExPos = random.randint(player.getCoords()[0],player.getCoords()[0]+100)
    lvl2EyPos = math.hypot(lvl2radius, lvl2ExPos)

    lvl3radius = random.randint(1,5)
    lvl3ExPos = random.randint(player.getCoords()[0],player.getCoords()[0]+100)
    lvl3EyPos = math.hypot(lvl3radius, lvl3ExPos)

    while gameTime <= 30000:
        for nolvl1 in range(5):
            lvl1EnemyRect = pygame.Rect(lvl1ExPos, lvl1EyPos, lvl1EW, lvl1EH) #initalises the enemies
            enemyList.append(lvl1EnemyRect)


    lvl1velRatio = 5   

    if totaldistlvl1 !=0:
        lvl1ExPos += lvl1velRatio * distXlvl1 / totaldistlvl1
        lvl1EyPos += lvl1velRatio * distYlvl1 / totaldistlvl1    
    ...
    while True:
        ...
        if pygame.Rect.colliderect(lvl1EnemyRect, lvl2EnemyRect) or pygame.Rect.colliderect(lvl1EnemyRect, lvl3EnemyRect):
                del lvl1EnemyRect
                lvl1EnemyRect = pygame.Rect(random.randint(50,950), random.randint(50,700), lvl1EW, lvl1EH)
                
        if pygame.Rect.colliderect(lvl1EnemyRect, lvl2EnemyRect) or pygame.Rect.colliderect(lvl2EnemyRect, lvl3EnemyRect):
            del lvl2EnemyRect
            lvl2EnemyRect = pygame.Rect(random.randint(50,950), random.randint(50,700), lvl2EW, lvl2EH)
            
        if pygame.Rect.colliderect(lvl1EnemyRect, lvl3EnemyRect) or pygame.Rect.colliderect(lvl2EnemyRect, lvl3EnemyRect):
                del lvl3EnemyRect
                lvl3EnemyRect = pygame.Rect(random.randint(50,950), random.randint(50,700), lvl3EW, lvl3EH)
...
run = True
while run:
    titleScreen() #change back to title screen when done
pygame.quit() 
